...
%winding
\draw (0.6,1.4) to [out=45,in=0] (0.2,1.5) to (-1,1.5) node[left]{$+$};
...
\draw[-latex](-0.5,1.6)--++(0.4,0)node[pos=0.5,above]{$i$};

%flux
\draw[gray,dashed,-latex](1.1,1.8)node[left]{$\phi_c$}--++(0.2,0)--++(0,-\height+\thick)--++(-\width+\thick,0)--++(0,\height-\thick)--++(0.3,0);
%urdu coil
...
\draw[gray,dashed](1.1,1)--++(\thick,0)--++(\depthX,\depthY)--++(-\thick,0)--cycle;
\draw[gray,dashed,<-] (1.6,1.2) to [out=90,in=-90](2.4,1.7)node[above right,black]{$A_c=b w$};
\draw[gray,dashed,<-](1.3,0.6) to [out=0,in=-180] (2.7,1)node [right,black]{$l_c$ لمبائی اوسط پر لکیر دار نقطہ };
\end{tikzpicture}
\end{urdufont}
\end{document}
%---------------------
