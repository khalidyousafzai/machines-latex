...
\pgfmathsetmacro{\tickL}{0.1}    %tick mark length
...
%cube
\draw[gray,thin] (n1)--(n2);   %front side
\draw[gray,thin] (n2)--(n6)--(n7)--(n3)--cycle;   %top side
\draw[gray,thin] (n3)--(n7)--(n8)--(n4);     %right side
%vectors
\draw[-latex] (0,0,0)--(\x,\y,\z)node[pos=0.5,above left]{${\bf{A}}$} node[above]{$P$};
\draw[-latex] (0,0,0)--(\x,0,0)node[pos=0.2,left]{${\bf{A}}_x$};
\draw[-latex] (\x,0,0)--(\x,\y,0)node[pos=0.5,below]{${\bf{A}}_y$};
\draw[-latex] (\x,\y,0)--(\x,\y,\z)node[pos=0.7,right]{${\bf{A}}_z$};
%TEXT
\draw node at (0,\y+2,\z){$\begin{aligned} &P(x,y,z)\\  \kvec{A}&=\kvec{A}_x +\kvec{A}_y+\kvec{A}_z\\   &=x \ax+y \ay+z\az \end{aligned}$};
%lines on body
%\foreach \delX in {0,...,\x}{
